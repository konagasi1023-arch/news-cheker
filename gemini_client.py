...
def renumber_articles(text: str) -> str:
    ...
    # 「百一番目。」のように「番目」で書かれることもある（2026-09-19、軽量モデルに
    # 切り替わった回で6件）。数えられずに検算も分割も記事を見失うので、揃える。
    # ただし「番目」は直後が「。」のときだけ（「百一番目。記事の題名は…」の形）。
    # 本文の「九番目の未認知以外のセグメントは…」まで拾って番号を振り、
    # 後ろの記事の番号が全部1つずれた（2026-09-25、Wisdom-Beta 604件で605個）。
    return re.sub(r"(\A|\n)[0-9〇一二三四五六七八九十百]+(?:件目|番目(?=。))",
                  replace, text)
