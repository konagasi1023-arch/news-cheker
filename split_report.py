...
# 1ファイルの上限（原稿の読み上げ分数）。
#
# 経緯：30分（2026-08）→45分（2026-09-07、ファイル数を抑えるため）→20分（2026-09-17）
# →**38分**（2026-09-20、ユーザーが「次からはこの粒度で」と指定）。
# 38分だと1パート14件前後、音声1本（30〜40分）で1件あたり約2.5分。
# 20分（10件前後）は本数が多すぎた：89件で13本になり、ユーザーが手で半分にした。
# 音声解説の出力は「長め」でも30〜40分前後なので、45分ぶんの原稿（20〜26件）を渡すと
# 1件70〜90秒に圧縮される。ユーザーの要件は「要約はよいが今の2〜3倍、題名だけはNG」で、
# 本数と詳しさの取り合いで、ユーザーが選んだ釣り合いがここ。
# もっと詳しくするなら 20（10件前後・1件3分前後）、本数を減らすなら 45（20〜26件）。
DEFAULT_MAX_MINUTES = 38
